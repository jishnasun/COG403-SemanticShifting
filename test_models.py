...
# ===============================
# MODEL ABLATION FUNCTION
# ===============================

def build_P_T_given_S(mode):
    """
    mode = "full"        -> P(T|S) ∝ P(S|T) * P(T)
    mode = "prior"       -> P(T) only
    mode = "similarity"  -> P(S|T) only
    """

    if mode == "full":
        print("\nRunning FULL Bayesian model")
        P = P_S_given_T * P_T

    elif mode == "prior":
        print("\nRunning PRIOR ONLY model")
        # same distribution for every source
        P = np.tile(P_T, (n_concepts, 1))

    elif mode == "similarity":
        print("\nRunning SIMILARITY ONLY model")
        P = P_S_given_T.copy()

    else:
        raise ValueError("Unknown mode")

    # normalize rows → valid probability distribution
...
def evaluate_model(P_T_given_S, label):
...
# run evaluation so it can be called
def run_eval(label):
    p = build_P_T_given_S(label)
    results = evaluate_model(p, label)
    return results
...
evaluate_model(P_prior, "PRIOR ONLY  P(T)")

P_sim = build_P_T_given_S("similarity")
evaluate_model(P_sim, "SIMILARITY ONLY  P(S|T)")
